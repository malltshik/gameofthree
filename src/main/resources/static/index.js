...
app.controller("GameCtrl", ["$rootScope", "$scope", '$location', '$anchorScroll', function ($rootScope, $scope, $location, $anchorScroll){
    ...
    $scope.joinGame = function (game) {
        ...
        $rootScope.ws.subscribe(`/user/queue/game/${game.id}/moves`, function (message) {
            $rootScope.$apply(function () {
                $rootScope.game = JSON.parse(message.body);
                if(!$rootScope.game.winner && $scope.autoplay) {
                    $scope.randomMove()
                }
